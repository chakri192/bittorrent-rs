...
pub fn fetch_metadata_from_peer(addr: SocketAddr, info_hash: [u8; 20], our_peer_id: [u8; 20], timeout: Duration, encryption: crate::peer::Encryption, transport: &crate::peer::Transport) -> Result<Vec<u8>, MetadataFetchError> {
    let (mut stream, peer_handshake) = crate::peer::connect_and_handshake_with(addr, info_hash, our_peer_id, true, false, timeout, encryption, transport)?;
...
mod tests {
    ...
    fn fetches_and_verifies_metadata_end_to_end() {
        ...
        let fetched = fetch_metadata_from_peer(addr, info_hash, [0x33; 20], Duration::from_secs(5), Default::default(), &Default::default()).unwrap();
    ...
    fn fetched_metadata_builds_a_valid_torrent_file_via_the_adapter() {
        ...
        let fetched = fetch_metadata_from_peer(addr, info_hash, [0x44; 20], Duration::from_secs(5), Default::default(), &Default::default()).unwrap();
    ...
    fn a_peer_claiming_an_absurd_metadata_size_is_refused_before_anything_is_allocated() {
        ...
        for size in [-1, 0, i64::MIN, crate::metadata::MAX_METADATA_SIZE + 1, i64::MAX] {
            ...
            let result = fetch_metadata_from_peer(addr, [0x11; 20], [0x22; 20], Duration::from_secs(2), Default::default(), &Default::default());
