...
mod tests {
    ...
    /// An HTTP tracker on loopback that answers announces with `peer`, and
    /// records each request line.
    fn tracker(peer: SocketAddr) -> (String, Arc<Mutex<Vec<String>>>) {
        let listener = TcpListener::bind("127.0.0.1:0").unwrap();
        ...
        thread::spawn(move || {
            for stream in listener.incoming() {
                ...
                let SocketAddr::V4(v4) = peer else { unreachable!() };
