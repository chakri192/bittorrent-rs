...
<p>
  ...
  <img alt="Tests" src="https://img.shields.io/badge/tests-1086%20passing-1c1c1e?style=flat-square" />
...
Removing a torrent never deletes what was downloaded. A torrent whose `.torrent` file has gone, or that cannot be read, is listed as `failed`, with the reason, and can be removed. Whole torrents only: there is no file selection, no pause, and no per-torrent rate limit.
...
./scripts/interop-aria2.sh        # against a real client, aria2, if installed: plain and encrypted, both directions, magnet metadata (INTEROP_LSD=1 adds local discovery)
```

1086 tests (1084 unit, 1 integration, 1 doctest), all passing. They stay on loopback, with one exception: a test of local discovery uses the real multicast group if the machine has one, and says so if it has not.
...
- **Encryption has met one other client.** MSE is implemented from the specification, verified against Python's big-integer arithmetic, RC4's published vectors and a second implementation written for the purpose, and works with aria2 1.37 both ways with encryption required; libtorrent-based clients and others have not been tried, so it stays opt-in.
...
- **The daemon has only met loopback.** Two daemons exchange torrents on one port, and the real binary does the same against fake peers and trackers in the harness; it has not run for long against the public swarm, and nothing has been tried across many torrents at once beyond two. It is Unix-only (its control socket is a Unix socket). Its torrents are whole ones, without selection, pause or limits of their own; a torrent that had reached a seed limit is seeded again after a restart, until it reaches the limit again; and every torrent's lookups share one DHT thread per family, so a slow lookup delays the others' by up to ten seconds.
