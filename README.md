...
<p>
  ...
  <img alt="Tests" src="https://img.shields.io/badge/tests-1369%20passing-1c1c1e?style=flat-square" />
  <img alt="BEPs" src="https://img.shields.io/badge/BEP-3%20·%205%20·%206%20·%209%2F10%20·%2011%20·%2012%20·%2014%20·%2015%20·%2019%20·%2021%20·%2027%20·%2029%20·%2032%20·%2042%20·%2043%20·%2044%20·%2046%20·%2047%20·%2048%20·%2052%20·%2053%20·%2055-1c1c1e?style=flat-square" />
...
| 44 | Storing arbitrary data in the DHT | `get`/`put` for immutable items (content-addressed by the SHA-1 hash of their bencoded value) and mutable ones (addressed by an ed25519 public key and optional salt, updated in place by sequence number, with `cas` compare-and-swap): this node answers both for other nodes (a bounded, two-hour-expiring store, the same "downloader first, storage node second" stance as BEP 5's own peer store) and issues `get` lookups of its own for BEP 46, below. Every accepted value is verified -- an immutable one against the target hash, a mutable one against its signature and that its key truly hashes to the target asked for -- before being trusted, whichever side of the exchange this node is on. Signature math uses `ed25519-dalek` rather than a hand-rolled implementation (the one cryptographic primitive in this project not written from the spec, unlike SHA-1/SHA-256/RC4: getting elliptic-curve verification wrong is a security hazard, not just a correctness bug); the buffer a signature covers, and the target-hashing, are this project's own and are checked byte for byte against BEP 44's own published test vectors, keys and signatures included. This client does not itself author or `put` a mutable item of its own (it only relays other publishers' and reads them) |
| 46 | Updating Torrents Via DHT Mutable Items | Resolves a `magnet:?xs=urn:btpk:<public key>&s=<salt>` pointer to the info hash it currently names, via a BEP 44 `get` on `sha1(public key + salt)` and reading `v`'s `ih` field back out, checked against the BEP's own published target-ID vectors. The DHT-facing half (`Dht::resolve_torrent_pointer`) is built and tested; it is not yet wired into the `download` binary's own argument parsing, so a `urn:btpk:` link cannot be handed to `download` on the command line today -- everything up to that last connection is done |
...
| 14 | Local Service Discovery | Announces the torrent to the multicast group `239.192.152.143:6771` every five minutes and dials whoever announces the same one there; not for private torrents. **IPv6 joins a second group, `ff15::efc0:988f`, on the same schedule and port** (the amendment most clients honour), on every up, multicast-capable network interface (an IPv6 join on `0`/"any" is refused by macOS and BSD, unlike Linux, so real interfaces are found and each tried); the IPv6 half follows the run's own `--ipv6`/`--no-ipv6`/route-probe choice, not a config default. Checked over loopback, against the BEP's message format, and between two sockets on the real group on one machine (which took several seconds to deliver the first datagram), and against aria2's local peer discovery in both directions over the real group (a client that starts later is answered at once, not five minutes later); **not between machines** or with any other client. One development machine's kernel was found to join the IPv6 group but refuse to route a *send* to it on any interface at all (a deprecated address class it may simply not carry a route for) -- receiving from a peer on a network that does route it is unaffected, but this client's own IPv6 announcements never left that particular machine |
| 29 | uTP | A reliable stream over UDP with LEDBAT congestion control (a 100 ms queuing-delay target), selective acks and retransmission; dialed and accepted, sharing the DHT's UDP port. **Opt-in** (`--transport`), and, on loopback, **against Transmission 4.1.3's uTP (libutp) and libtorrent 2.1.1's, in both directions** (`scripts/interop-transmission.sh`, `scripts/interop-libtorrent.sh`); not yet across a real network. **A second socket takes inbound uTP over IPv6 too** when `--ipv6` is in effect, on the same port where one can be had -- an IPv6 peer can uTP-dial in; outbound uTP dialing stays on the IPv4 socket alone, so this client always reaches out over TCP or IPv4 uTP, never IPv6 uTP, to a peer it dials itself |
| 42 | DHT Security Extension | A node's id is made from its external address: the first 21 bits are a CRC-32C of the address (masked as the BEP says, IPv4 and IPv6) and the last byte is the random number that went into it. Every answer says the address the query came from (`ip`), and once **five different nodes agree** where this node is, its id is made anew from that address and its routing table centred on it. Checked against the BEP's five example ids, and **against the live DHT**: within 8 seconds nodes told this client its public address, the same one a web service reports. Ids other nodes have are *not* checked (`secure_id::is_valid` exists, but many nodes in the wild are not compliant, so nothing is refused for it) |
| 43 | Read-only DHT Nodes | A query marked `ro` is answered, and its sender is not put in the routing table. The client can itself be read-only (`Dht::set_read_only`: it then marks its queries and answers none), but nothing switches that on: a client cannot tell whether it is reachable |
...
| 48 | Tracker Protocol Extension: Scrape | `download --scrape` asks each tracker for the seeders, leechers and completed downloads of a torrent without joining it: the scrape URL is the announce URL with its last segment `announce…` made `scrape…` (query kept), over HTTP, HTTPS or UDP. Tried against Debian's public tracker (which answered) and against a fake one, not against others |
...
| 55 | Holepunch Extension | A peer this client is connected to can ask it to introduce two other peers that cannot reach each other directly: on a `rendezvous` naming a target this client is also connected to (and who also offers the extension), it relays a `connect` to that target and answers the sender with one too, each naming the other's endpoint; on receiving a `connect` itself (as either the introduced peer or the one who asked), it dials the named endpoint over uTP (as the BEP requires) unless already connected to it. This client also asks a connected, capable peer to relay for it whenever a fresh dial comes back unreachable. Built for connections actively downloading a torrent only -- a connection kept purely to serve a finished torrent does not act as a relay. `NoSelf` detection (a target naming this client's own address) is not wired up; such a target is treated as simply not connected, which the BEP allows. Checked against its own wire format byte for byte and against every branch of the relay/reactive-dial logic; not yet against another client's holepunch |
...
| `--scrape` | Ask the torrent's trackers how many seeders and leechers it has and how often it has been finished (BEP 48; UDP trackers by BEP 15), print each tracker's answer and exit: no peer is looked for and nothing is downloaded. Works on a `.torrent` file or a magnet link (its `tr` trackers); a tracker that does not answer within 20 seconds, or has no scrape (its announce URL must end in `announce`), is listed as such, and the run fails only if none answers. `--json` gives one `scrape` event per tracker |
...
./scripts/interop-libtorrent.sh   # against libtorrent, if its Python bindings are installed: v1 and v2, TCP/uTP/encryption, magnet links, hash requests, padding files, both directions on one connection
...
1369 tests (1367 unit, 1 integration, 1 doctest), all passing. They stay on loopback, with one exception: a test of local discovery uses the real multicast group if the machine has one, and says so if it has not.
...
- **Every connection goes both ways, and choking is tit-for-tat, but only lightly tested against other clients.** A peer this client dials and downloads from is also served on that connection: told what pieces it has (first, as BEP 3 has it, and the new ones as they are verified), unchoked, and answered when it asks. A peer that connects to this client is served, and, if it says it has a piece this client still lacks, is downloaded from over the same connection; one that has nothing to give (or never unchokes) is kept and served. Four peers are unchoked at a time, three chosen by how much they gave in the last ten seconds (then by how much they took) and one optimistic slot that moves every thirty; a seed, which is given nothing, ranks by what they took. Checked against libtorrent 2.1.1 in both directions (it and this client each had half a torrent, over a connection each side made in turn: `scripts/interop-libtorrent.sh`); other clients' reciprocation is untested. A peer is picked up from an inbound connection only when it says (`bitfield`, `have all`, `have`) that it has a piece wanted, and a peer that gets one later is picked up then; a peer that connected and sent a piece that failed its hash is not downloaded from again (by address, until the run ends) though it is still served; a connection is not handed back to the seeder's loop when the download ends unless it was one a peer made.
...
- **Local discovery has met one other client, on one machine.** It finds and is found by aria2 over the real multicast group, in both directions, though the first datagram between fresh sockets has taken several seconds; it has not been tried between machines. A second socket joins the IPv6 group too (BEP 14's amendment) when IPv6 is in use, though this development machine's kernel refuses to *send* to it on any interface (receiving is unaffected; see the BEP 14 row above). `cargo test --lib lsd -- --nocapture` says whether the group works on yours. A client that starts after another hears it only when that one next announces (every five minutes), unless it answers newcomers, as this one does.
...
├── serving.rs        One connection's upload side: bitfield, `Have`s, who is unchoked, answers to requests; used by the seeder and by download workers
