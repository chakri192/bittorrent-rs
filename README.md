...
<p>
  ...
  <img alt="Tests" src="https://img.shields.io/badge/tests-1139%20passing-1c1c1e?style=flat-square" />
...
1139 tests (1137 unit, 1 integration, 1 doctest), all passing. They stay on loopback, with one exception: a test of local discovery uses the real multicast group if the machine has one, and says so if it has not.
...
- **Encryption has met two other clients.** MSE is implemented from the specification, verified against Python's big-integer arithmetic, RC4's published vectors and a second implementation written for the purpose, and works with aria2 1.37 and Transmission 4.1.3 both ways with encryption required; libtorrent-based clients have not been tried, so it stays opt-in.
...
- **uTP has met one other implementation, on one machine.** Its connection logic is tested against a simulated network with loss, reordering, duplication and a slow link, over loopback UDP, against a separately written Python endpoint, and against Transmission 4.1.3's libutp in both directions (3 MiB each way, Transmission's TCP switched off). Nothing has tested it over a real, lossy network or against a libtorrent client's uTP, so it stays opt-in; and inbound uTP needs the UDP port to equal the TCP port, which it does unless the preferred port was taken.
...
- **The daemon has only met loopback.** Two daemons exchange torrents on one port, the real binary does the same against fake peers and trackers in the harness, and aria2 downloads two torrents from its one port and is downloaded from by it, two at a time; it has not run for long against the public swarm, and nothing has been tried with more than two torrents at once. It is Unix-only (its control socket is a Unix socket). Every torrent's lookups share one DHT thread per family, so a slow lookup delays the others' by up to ten seconds.
