...
<p>
  ...
  <img alt="Tests" src="https://img.shields.io/badge/tests-975%20passing-1c1c1e?style=flat-square" />
  <img alt="BEPs" src="https://img.shields.io/badge/BEP-3%20·%205%20·%206%20·%209%2F10%20·%2011%20·%2014%20·%2015%20·%2019%20·%2027%20·%2029%20·%2052%20(partial)-1c1c1e?style=flat-square" />
...
| 52 | BitTorrent v2 | The file tree, SHA-256 merkle trees over 16 KiB blocks and the piece layers, read from `.torrent` files (v2 only, or hybrid, which is downloaded through its v1 hashes), made by `create_torrent --v2`, and checked by `--verify` piece by piece. Trees and the SHA-256 are checked against Python's `hashlib`. **A v2-only torrent cannot yet be downloaded or seeded**: that needs the hash-request messages and a v2 piece layout |
...
| `--v2` | Makes a BitTorrent v2 torrent (BEP 52): a file tree with a SHA-256 merkle root per file and the piece layers, no v1 piece hashes. The piece length must then be at least 16K. The client can list and `--verify` such a torrent but cannot yet download it |
...
- **BitTorrent v2 is only partly there.** v2 torrents can be made, read, listed and verified, but a v2-only torrent cannot be downloaded or seeded, and a magnet link with only a v2 hash (`btmh`) is not followed: both need the v2 hash-request messages and a piece layout in which files start on piece boundaries. Hybrid torrents work, through their v1 hashes.
