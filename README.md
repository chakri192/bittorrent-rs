...
<p>
  ...
  <img alt="Tests" src="https://img.shields.io/badge/tests-500%20passing-1c1c1e?style=flat-square" />
...
Ctrl-C and `SIGTERM` stop the client cleanly whether or not there is a terminal: the listener and DHT stop, the router's port mapping is removed, and the resume file is kept. Every connection is shut down at once, so a peer that has gone silent does not delay the stop. A second signal exits immediately (status 130).
