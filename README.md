...
<p>
  ...
  <img alt="Tests" src="https://img.shields.io/badge/tests-1166%20passing-1c1c1e?style=flat-square" />
...
./scripts/interop-libtorrent.sh   # against libtorrent, if its Python bindings are installed: v1 and v2, TCP/uTP/encryption, magnet links, hash requests
...
1166 tests (1164 unit, 1 integration, 1 doctest), all passing. They stay on loopback, with one exception: a test of local discovery uses the real multicast group if the machine has one, and says so if it has not.
...
- **BitTorrent v2 is complete but for two things, and has met one other implementation.** Hash requests, v2 magnet links and everything else in the table above work against libtorrent 2.1.1 on loopback, in both directions; not against another v2 client (there are few), nor across a real network. A seed refuses requests for the 16 KiB leaf hashes (BEP 52 allows that, apart from a request right after a block of that range was served, which no client has sent here), and web seeds are not used for v2 torrents. A peer that gives the piece layers wrongly is refused and another tried; a torrent none can supply them for is given up on after 90 seconds. Hybrid torrents work, through their v1 hashes.
