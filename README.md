...
<p>
  ...
  <img alt="Tests" src="https://img.shields.io/badge/tests-1213%20passing-1c1c1e?style=flat-square" />
...
1213 tests (1211 unit, 1 integration, 1 doctest), all passing. They stay on loopback, with one exception: a test of local discovery uses the real multicast group if the machine has one, and says so if it has not.
...
- **Tit-for-tat works on the connections this client makes, not on those made to it.** A peer this client dials and downloads from is also served on that connection: told what pieces it has (first, as BEP 3 has it, and the new ones as they are verified), unchoked, and answered when it asks. Four peers are unchoked at a time, three chosen by how much they gave in the last ten seconds (then by how much they took) and one optimistic slot that moves every thirty; a seed, which is given nothing, ranks by what they took. Peers that connect *to* this client are served but never downloaded from, so what they might give goes unclaimed. Checked against libtorrent 2.1.1, which each had half a torrent of and got the other half of over the one connection this client made (`scripts/interop-libtorrent.sh`); other clients' reciprocation is untested.
