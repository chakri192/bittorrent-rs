...
<p>
  ...
  <img alt="Tests" src="https://img.shields.io/badge/tests-849%20passing-1c1c1e?style=flat-square" />
  <img alt="BEPs" src="https://img.shields.io/badge/BEP-3%20·%205%20·%206%20·%209%2F10%20·%2011%20·%2014%20·%2015%20·%2019%20·%2027-1c1c1e?style=flat-square" />
...
| 14 | Local Service Discovery | Announces the torrent to the multicast group `239.192.152.143:6771` every five minutes and dials whoever announces the same one there; IPv4 only, and not for private torrents. Checked over loopback and against the BEP's message format, but **the multicast group itself has not been exercised end to end** |
...
- **Local discovery is IPv4-only and has met no other client.** It works over loopback, but the multicast group could not be exercised where this was built (a Mac whose system Python could use multicast and whose freshly built binaries could not), so whether it finds a real neighbour is unconfirmed. `cargo test --lib lsd -- --nocapture` says whether it can on yours.
